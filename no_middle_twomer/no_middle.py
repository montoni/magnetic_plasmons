...
for epsb in range(1,2):
	...
	for r in range (10,301):
		...
		Loc = [np.array([-e2, 2*e1]),
			   np.array([-2*e2, e1]),np.array([-2*e2, -e1]),
			   np.array([-e2, -2*e1]),
			   np.array([e2,-2*e1]),np.array([2*e2,-e1]),
			   np.array([2*e2, e1]),np.array([e2, e1])] #location vectors for center of each sphere - they go counterclockwise, with one being the top center particle and six being the bottom center particle.
		...
		for mode in range(0,2):
			...
			plt.show() '''         
			if mode == 0:
				NN.append(eigen[(2*numPart)-1])
				
			else:
				NS.append(eigen[(2*numPart)-2])
