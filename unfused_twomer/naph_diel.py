...
numPart = 12; #number of particles
...
for epsb in range(1,2):
	...
	for r in range (10,301):
		...
		Loc = [np.array([-e2, 2*e1]),np.array([-e2, 4*e1]),
			   np.array([0, 5*e1]),np.array([e2, 4*e1]),
			   np.array([e2, 2*e1]),np.array([0, e1]),
			   np.array([-e2, -2*e1]),np.array([-e2, -4*e1]),
			   np.array([0, -5*e1]),np.array([e2, -4*e1]),
			   np.array([e2 , -2*e1]),np.array([0 , -e1])] #location vectors for center of each sphere - they go counterclockwise, with one being the top center particle and six being the bottom center particle.
		center = [np.array([0,3*e1]),np.array([0,-3*e1])]
		...
		for mode in range(0,2):
			...
			vec = np.reshape(eigenVectors[:,2*numPart-(mode+1)],[12,2])
			'''x,y = zip(*Loc)
			u,v = zip(*vec)
			...
			for cent in range(0,2):
				...
				for part in range(0,numPart):
					...
					if np.sqrt(disp[0]**2+disp[1]**2) < 2.1*e1:
...
np.savetxt('NN_corners',NN)
np.savetxt('NS_corners',NS)