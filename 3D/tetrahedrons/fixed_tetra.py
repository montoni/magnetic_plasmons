...
for r in range(1,31):
	...
	Loc = [np.array([0,0,0]),np.array([ex,ey,0]),np.array([ex,-ey,0]),np.array([2*ex/3,0,ez])],
		   #np.array([sep*rij+ex*2,0,0]),np.array([sep*rij+ex,ey,0]),np.array([sep*rij+ex,-ey,0]),np.array([sep*rij+ex+ex/3,0,ez])]
	'''Loc.append(np.array([2*ex,0,0]))
	Loc.append(np.array([ex,ey,0]))
	Loc.append(np.array([ex,-ey,0]))
	Loc.append(np.array([3*ex/2,0,ez]))'''
	'''x,y,z = zip(*Loc)
	fig = plt.figure()
...
plt.figure()
plt.plot(sep,modes[0],sep,modes[1],sep,modes[2],linewidth=3)
