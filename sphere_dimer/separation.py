...
for sep in range(0,101):
	...
	for num in range(0,4):
		while np.sqrt(np.square(w_0*hbar/elec - eigen[2*numPart-(num+1)])) > 0.00000001:
			...
			eigen=(hbar*wsp/elec) * np.sqrt(eigenValues) # the eigenvalues have units of energy^2, so we take the square root
			print eigen
...
plt.xlim([1,30])
plt.savefig('dimer_eigenmodes_reduced_alpha.pdf')
plt.show()
