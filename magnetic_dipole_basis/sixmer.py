...
nn_third = np.loadtxt('../unfused_twomer/NN_corners')
ns_third = np.loadtxt('../unfused_twomer/NS_corners')
...
for r in range(10,301):
	...
	Loc_mag = [np.array([0,0,0]),np.array([2*e2,0,0]),np.array([e2,-3*e1,0]),
			   np.array([-e2,-3*e1,0]),np.array([-2*e2,0,0]),
			   np.array([-e2,3*e1,0]),np.array([e2,3*e1,0])]
	...
	coupling_corner = nn_third[index]-ns_third[index]
	for mode in range(1,numRings+1):
		...
		while np.sqrt(np.square(w_mag*hbar/elec - eigen_mag[numRings-mode])) > 0.00000001:
			...
			eigen_mag=(eigenValues_mag) # the eigenvalues have units of energy^2, so we take the square root
			#print eigen_mag
		#print eigenVectors_mag
		mag_vec = eigenVectors_mag[:,mode-1]
		print mag_vec
		if abs(np.sum(mag_vec)) > 1e-2:
...
if len(dipole) > len(nodipole):
	nodipole.append(dipole[index])
	del(dipole[index])
...
plt.plot(r,all_N,r,excited,r,alt_NS,r,dipole[:,0],r,nodipole[:,0],linewidth=3)	
