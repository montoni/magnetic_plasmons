...
net_magnetization = []
mie_omegas = np.loadtxt('../eps_10_mie_omegas.txt')
nn_near = np.loadtxt('../dielectric_study/NN_eps_10.txt')
ns_near = np.loadtxt('../dielectric_study/NS_eps_10.txt')
...
for r in range(10,301):
	...
	epsb = 10; # background dielectric - right now we're in vacuum
		...
		for n in range (0,2*numPart):
			for m in range (n,2*numPart):
				if m == n: #if m and n are the same, the hammy gets the plasmon energy
					...
					ge = (np.sqrt(epsb))*(r_unit *space_cos* (p_dot_p - p_nn_p) + (r_cubed*space_cos + r_squared*space_sin) * (3*p_nn_p - p_dot_p)) #this is p dot E
			...
			for n in range (0,numRings):
				for m in range (n,numRings):
					...
					if Rmag == 0:
						...
						H_mag[n,m] = (w_mag_0*hbar/elec)*((coupling_near))
					 #this has the minus sign we need.
					#elif Rmag == 4*e2:
	...
	net_magnetization.append(0.5*abs(np.sum(eigenVectors_mag[:,numRings-1])))
...
plt.plot(r,net_magnetization,linewidth=3)
plt.xlabel('Radius (nm)')
plt.ylabel('Net Magnetization')
plt.show()
np.savetxt('chain_magnetization_eps10',net_magnetization)
