...
p1 = shift(rot(rod,30,[0,0,1]),[-sqrt(3)*dist/2-dist/2,dist/2,0]);
p2 = shift(rot(rod,150,[0,0,1]),[-sqrt(3)*dist/2-dist/2,-dist/2,0]);
p3 = shift(rot(rod,90,[0,0,1]),[-dist/2,0,0]);
p4 = shift(rot(rod,90,[0,0,1]),[dist/2,0,0]);
p5 = shift(rot(rod,30,[0,0,1]),[sqrt(3)*dist/2+dist/2,-dist/2,0]);
p6 = shift(rot(rod,150,[0,0,1]),[sqrt(3)*dist/2+dist/2,dist/2,0]);
...
A(:,2) = ext;
A(:,3) = sca;
fid = fopen(strcat('rods_narrow_NS_',num2str(rad),'nm'),'wt');
           for j = 1:length(enei)
           fprintf(fid,' %g', A(j,1));
           fprintf(fid,' %g', A(j,2));
           fprintf(fid,' %g', A(j,3));
