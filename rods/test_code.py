...
for rad in radius:
	...
	ex = inter_particle_dist * math.sqrt(3)/2
	ey = inter_particle_dist * .5
	#center = np.zeros((2,numRings))
	center = [np.array([-ey-ex/3,0]),np.array([ey+ex/3,0])]
	# quick loop to place particles in the world
	Loc = [np.array([-ey,ey]),np.array([-ey-ex,0]),np.array([-ey,-ey]),np.array([])]
