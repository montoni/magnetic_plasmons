...
for r in range(1,31):
	...
	'''Loc = [np.array([-ey,ey]),np.array([-ey-ex,0]),np.array([-ey,-ey]),
		   np.array([ey,-ey]),np.array([ey+ex,0]),np.array([ey,ex])]'''
	'''x,y,z = zip(*Loc)
	fig = plt.figure()
		...
		while np.sqrt(np.square(w_0*hbar/elec - eigen[(2*numPart)-(mode+1)])) > 0.0000001:
			...
			for n in range (0,2*numPart):
				for m in range (0,2*numPart):
					...
					elif np.sqrt(np.sum(np.square(Loc[(n/2)]-Loc[(m/2)]))) < a0: #if m and n are on the same particle, they don't couple
						H[n,m] = 0
...
plt.figure()
plt.plot(r,modes[0],r,modes[1],r,modes[2],r,modes[3],linewidth=3)
plt.show()
