...
numPart = 22; #number of particles
...
for epsb in range(1,2):
	...
	for r in range (20,21):
		...
		Loc = [np.array([-5*e2,2*e1]),np.array([-6*e2,e1]),np.array([-6*e2,-e1]),np.array([-5*e2,-2*e1]),
			   np.array([-2*e2, e1]),np.array([-3*e2, 2*e1]),
			   np.array([-4*e2, e1]),np.array([-4*e2, -e1]),np.array([-3*e2, -2*e1]),np.array([-2*e2, -e1]),
			   np.array([-1*e2, -2*e1]),np.array([0*e2, -e1]),np.array([0*e2 , e1]),np.array([-1*e2 , 2*e1]),
		       np.array([1*e2,-2*e1]),np.array([2*e2,-e1]),np.array([2*e2,e1]),np.array([1*e2,2*e1]),
		       np.array([3*e2,-2*e1]),np.array([4*e2,-e1]),np.array([4*e2,e1]),np.array([3*e2,2*e1])]#,
		       #np.array([5*e2,-2*e1]),np.array([6*e2,-e1]),np.array([6*e2,e1]),np.array([5*e2,2*e1])]#,
		       #np.array([9*e2,-2*e1]),np.array([10*e2,-e1]),np.array([10*e2,e1]),np.array([9*e2,2*e1]),
		       #np.array([11*e2,-2*e1]),np.array([12*e2,-e1]),np.array([12*e2,e1]),np.array([11*e2,2*e1]),
		       #np.array([13*e2,-2*e1]),np.array([14*e2,-e1]),np.array([14*e2,e1]),np.array([13*e2,2*e1])] #location vectors for center of each sphere - they go counterclockwise, with one being the top center particle and six being the bottom center particle.
		...
		for mode in [0,1,2,3,4]:
			...
			while abs(w_0*hbar/elec - eigen[(2*numPart)-(mode+1)]) > 0.00001:
				...
				eigenValues = np.real(w[idx]) # sorting
				eigenVectors = np.real(v[:,idx]) # sorting
			...
			#np.savetxt('fivemer_big_dipole_y',vec)
			u,v = zip(*vec)
			x,y = zip(*Loc)
			plt.figure()
			plt.quiver(x,y,u,v)
			...
			for number in range(0,numPart):
				...
				for phi in np.linspace(0,2*math.pi,poynting_points):
					...
					for theta in np.linspace(0,math.pi,poynting_points):
						...
						Bfield[theta_count,phi_count] = np.sqrt(alpha*hbar*w_0)*(wavenumber**2)*np.cross(nhat_dip,vec[number])*np.exp(1j*wavenumber*rmag) #
		...
		for idx1 in range(poynting_points):
			for idx2 in range(poynting_points):
				poynting[idx1,idx2] = np.linalg.norm(0.5*np.real(np.cross(Efield_total[idx1,idx2],np.conj(Bfield_total[idx1,idx2]))))
