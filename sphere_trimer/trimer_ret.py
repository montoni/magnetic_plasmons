...
for r in range(10,301):
	...
	Loc = [np.array([e1, 0]),np.array([-e1, 0]),np.array([0, e2])]
	...
	for num in range(0,1):
		while np.sqrt(np.square(w_0*hbar/elec - eigen[2*numPart-(num+1)])) > 0.0000001:
			...
			eigen=np.sqrt(eigenValues)*((hbar/elec)*wsp) # the eigenvalues have units of energy^2, so we take the square root
			print eigen
