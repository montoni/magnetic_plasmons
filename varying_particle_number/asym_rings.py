...
#numPart = [5,6] #number of particles in each ring, counting overlap - we will account later
...
for r in range(1,31):
	...
	for part_per_ring in numPart:
		...
		centers[count] = np.array([cent_dist*((-1)**(count+1)),0])
	
		#print centers
		
		for num in range(part_per_ring):
			if any(all(np.isclose(jjj, centers[count] + np.array([(part_to_cent*np.cos(phi+theta*(num))),(part_to_cent*np.sin(phi+theta*(num)))]))) for jjj in places):
				pass
			else:
				places.append(centers[count] + np.array([(part_to_cent*np.cos(phi+theta*(num))),(part_to_cent*np.sin(phi+theta*(num)))]))
		count += 1
	...
	for mode in range(0,2):
		...
		elif np.dot(vec[:,0],vec[:,1]) > 0:
			NS.append(eigen[2*numPart - (mode+1)])'''

#nearest = np.subtract(NN,NS)
plt.figure()
plt.plot(np.linspace(1,30,30),modes[0],np.linspace(1,30,30),modes[1],linewidth=3)
plt.legend('lowest','second')
plt.show()
