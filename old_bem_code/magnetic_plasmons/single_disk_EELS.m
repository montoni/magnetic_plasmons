...
p = comparticle( epstab, { p4 }, [ 2,1 ], 1 , op );
...
%  loss energies in eV
ene = [ 1.52, 2.43 ];%linspace( 2, 3.8, 81 );
...
Alice  = [Alice , Alice2];
 ...
 save(strcat('Map_single_disk_',num2str(ene(ien))),'Alice','-ascii')
...
figure(2)
colormap(cmapEfield)
EELSmap=load('Map_single_disk_2.43'); 
pcolor(EELSmap);
caxis([0 0.01])
shading interp
shading flat