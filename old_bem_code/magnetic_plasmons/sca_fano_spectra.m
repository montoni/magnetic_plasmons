...
cr  = 15 ;
pcube2 = asymcube2(2*cr, sublen(1), sublen(2), 15, [6, 6, 20, 20, 18 ] );
pcube2 = shift(pcube2,[sublen(1)/2-cr,sublen(1)/2-cr,-height/2]);

pcube3 = asymcube2(2*cr, 2*sublen(1), 2*sublen(2), 15, [6, 6, 30, 30, 30 ] );
pcube3 = shift(pcube3,[sublen(1)-cr,sublen(1)-cr,-height]);


 p = comparticle( epstab, {  p1, p2, p3, p4, p5, p6, pcube2, pcube3},  ... 
  [ 2,1; 2,1; 2,1; 2,1; 2,1; 2,1; 3,1; 3,1 ], 1,2,3,4,5,6,7,8, op );
