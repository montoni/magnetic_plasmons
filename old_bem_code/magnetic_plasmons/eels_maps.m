...
% HEIGHT OF RODS
rodedge = edgeprofile( height, 10, 'mode', '00', 'min', 0 );
hdata = struct( 'hmax',100);
% MAKE ROD POLYGON
rod_left  = polygon( 20, 'size', rodlen_left  ,'hdata', hdata); 
rod_right = polygon( 20, 'size', rodlen_right , 'hdata', hdata);
...
%  extrude rod
[ p, poly ] = tripolygon( rod_left, rodedge, 'hdata', hdata);
[ pr, polyr ] = tripolygon( rod_right, rodedge, 'hdata', hdata);
% extrude substrate
%brick = tripolygon( subs, subedge);
% MOVE AND ROTATE RODS
p1 = shift(pr, [radius1*cos(theta(1))-center, radius1*sin(theta(1)), h]);
...
[ x, y ] = meshgrid( linspace(  -40, 40, 81 ), linspace( -30, 30, 61 ) );
%  impact parameters
imp = [ x( : ), y( : ) ];
...
%Alice = [flipud(Alice3) ; Alice3];
 ...
 save(strcat('Map_VII_',num2str(ene(ien))),'Alice','-ascii')
